...
object PrefixTrie {
   ...
   */
  def apply[V](init: Map[String, V]): PrefixTrie[V] = {
    ...
    if (len > 1) {
      new NestedFixedDepthPrefixTrie(init)
    } else {
      new FixedDepthPrefixTrieValues(init)
    }
  }
}

/**
 * This trait represents a prefix trie that stores a mapping between string keys, and any values.
 *
 * @tparam V The type of the values stored in the trie.
 */
trait PrefixTrie[V] extends Serializable {
   ...
   */
  private[models] def find(key: String): TraversableOnce[(String, V)]
 ...
 * otherwise see docs for the PrefixTrie trait.
 *
 * @tparam V Type of the values stored by this class.
 *
 * @sa PrefixTrie
 */
private[models] class NestedFixedDepthPrefixTrie[V](init: Map[String, V]) extends PrefixTrie[V] {

  // the length of keys stored at this level of the nested tree
  val len = init.head._1.length

  // fixed length prefix map
  val prefixMap: Array[Option[PrefixTrie[V]]] = new Array(4)
  ...
  def find(key: String): TraversableOnce[(String, V)] = {
    // get the mapping bucket
    val bucket = cToI(key.head)
    val next = key.drop(1)
 ...
 * @sa PrefixTrie
 */
private[models] class FixedDepthPrefixTrieValues[V](init: Map[String, V]) extends PrefixTrie[V] {
